import { Meta, StoryObj } from '@storybook/react';

import { Box, Link, Typography } from '@mui/material';
import { SingleComment } from '../components/post/SingleComment';

const meta: Meta<typeof SingleComment> = {
  component: SingleComment,
  title: '4. Components/SingleComment ',
  parameters: {
    layout: 'centered'
  },
  tags: ['autodocs']
};

export default meta;
type Story = StoryObj<typeof SingleComment>;

export const Brand: Story = {
  args: {
    commentId: '1',
    username: 'john',
    isCreator: true,
    date: '07/07/2023',
    numLikes: 10,
    comment: 'hello world',
    profilePic: 'https://placekitten.com/250/250',
    allowReply: true
  }
};

Brand.storyName = `🔶 SingleComment`;

export const Spanish: Story = {
  args: {
    commentId: '1',
    username: 'Español',
    isCreator: true,
    date: '07/07/2023',
    numLikes: 10,
    comment: 'publicación',
    profilePic: 'https://placekitten.com/250/250',
    allowReply: true,
    likesText: 'Español',
    likeText: 'publicación',
    replyText: 'publicación'
  }
};

Spanish.storyName = `🔶 Spanish`;

export const Arabic: Story = {
  args: {
    commentId: '1',
    username: 'ارسل',
    isCreator: true,
    date: '07/07/2023',
    numLikes: 10,
    comment: 'ارسل',
    profilePic: 'https://placekitten.com/250/250',
    allowReply: true,
    likesText: 'ارسل',
    likeText: 'ارسل',
    replyText: 'ارسل',
    locale: 'ar'
  }
};

Arabic.storyName = `🔶 Arabic`;

const FigmaPreview = (props: any): JSX.Element => {
  return (
    <div
      style={{
        width: '90vw',
        height: '90vh'
      }}
    >
      <Box>
        {props.notImplemented && (
          <Typography variant="body1" gutterBottom sx={{ color: 'red' }}>
            NOT IMPLEMENTED
          </Typography>
        )}
        {props.inprogress && (
          <Typography variant="body1" gutterBottom sx={{ color: 'orange' }}>
            IN PROGRESS
          </Typography>
        )}
        {props.ok && (
          <Typography variant="body1" gutterBottom sx={{ color: 'green' }}>
            VERIFIED
          </Typography>
        )}
        <Link target="_blank" href={props.url} variant="body1">
          URL: {props.url}
        </Link>
      </Box>
      <iframe
        title="Figma Preview"
        style={{ border: '1px dotted black' }}
        width="100%"
        height="100%"
        src={props.url}
      ></iframe>
    </div>
  );
};

export const Figma: Story = {
  render: (): JSX.Element => (
    <FigmaPreview
      inprogress
      url="https://www.figma.com/embed?embed_host=share&url=https%3A%2F%2Fwww.figma.com%2Ffile%2Fl2mfmKnw7R89TAfSaz4p0O%2FFanfix-2.0-Design-System%3Ftype%3Ddesign%26node-id%3D243%253A5735%26mode%3Ddev"
    />
  )
};

Figma.storyName = '🎨 Figma';
